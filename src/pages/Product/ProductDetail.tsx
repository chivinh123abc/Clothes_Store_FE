    ...
    return (
      <Layout footer={<Footer />}>
        <div className="min-h-[60vh] flex flex-col items-center justify-center text-white px-4">
          <h2 className="text-4xl font-oswald font-black mb-6 italic uppercase tracking-tighter">Product Not Found</h2>
          <button
    ...
    <Layout footer={<Footer />} bleed={true}>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-10 lg:px-16 pt-32 pb-8">
        <div className="flex items-center gap-2 text-[10px] sm:text-xs font-oswald font-bold tracking-widest text-gray-500 uppercase">
          ...
          <ChevronRight size={12} className="text-white/50" />
          <span className="text-t1-red">{product.category}</span>
          <ChevronRight size={12} className="text-white/50" />
            ...
            >
              ...
              {product.soldOut && (
                <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-10">
                  <span className="px-10 py-5 text-4xl font-oswald font-black text-white border-4 border-white italic uppercase tracking-[0.2em]">SOLD OUT</span>
                </div>
              )}
          ...
          <div className="lg:col-span-5 flex flex-col">
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-8">
                {product.salePrice ? (
                  <>
                    ...
                    <span className="bg-t1-red/10 text-t1-red text-[10px] font-bold px-2 py-0.5 rounded border border-t1-red/20">SAVE {Math.round((1 - product.salePrice / product.price) * 100)}%</span>
                  </>
                ) : (
              ...
              <p className="text-gray-400 font-light leading-relaxed mb-10 italic">
                {product.description || 'Premium T1 merchandise designed for fans who demand excellence. This high-quality piece combines athletic performance with street-ready style.'}
              </p>

              {/* Size Selector */}
              <div className="mb-8">
                <div className="flex justify-between items-center mb-4">
                  <span className="text-xs font-oswald font-bold tracking-[0.2em] uppercase text-gray-400">SELECT SIZE</span>
              ...
              {/* Quantity Selector */}
              <div className="mb-10">
                <span className="text-xs font-oswald font-bold tracking-[0.2em] uppercase text-gray-400 block mb-4">QUANTITY</span>
                ...
                >
                  <AnimatePresence mode="wait">
                    {isAdded ? (
                      ...
                      >
                        <ShieldCheck size={20} /> ADDED TO CART
                      </motion.div>
                    ) : (
                      <motion.div
                        key="idle"
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-10 border-t border-t1-gray/20">
                <div className="flex flex-col items-center text-center gap-2">
                  ...
                  <span className="text-[10px] font-oswald font-bold tracking-widest text-white uppercase">FAST SHIPPING</span>
                  <span className="text-[10px] text-gray-500">2-5 Business Days</span>
                </div>
                <div className="flex flex-col items-center text-center gap-2">
                  <RotateCcw size={20} className="text-t1-red" />
                  <span className="text-[10px] font-oswald font-bold tracking-widest text-white uppercase">EASY RETURNS</span>
                  <span className="text-[10px] text-gray-500">30-Day Policy</span>
                </div>
                <div className="flex flex-col items-center text-center gap-2">
                  <ShieldCheck size={20} className="text-t1-red" />
                  <span className="text-[10px] font-oswald font-bold tracking-widest text-white uppercase">SECURE CHECKOUT</span>
          ...
          <div className="min-h-[200px] max-w-4xl italic text-gray-400 font-light leading-relaxed">
            {activeTab === 'description' && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                <p className="mb-6">
                  {product.description || 'Represent the world champions in style with this premium merchandise. Craftsmanship meets heritage in every stitch.'}
                </p>
                <p>
            ...
            {activeTab === 'specs' && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 md:grid-cols-2 gap-y-4">
                <div className="flex justify-between border-b border-t1-gray/10 pb-2 pr-10">
                  <span className="font-oswald text-xs tracking-widest text-white uppercase">Material</span>
                  <span className="text-xs uppercase">Premium Cotton Blend</span>
                </div>
                <div className="flex justify-between border-b border-t1-gray/10 pb-2 pr-10">
                  <span className="font-oswald text-xs tracking-widest text-white uppercase">Weight</span>
                  <span className="text-xs uppercase">320 GSM</span>
                </div>
                <div className="flex justify-between border-b border-t1-gray/10 pb-2 pr-10">
                  <span className="font-oswald text-xs tracking-widest text-white uppercase">Fit</span>
                  <span className="text-xs uppercase">Regular Oversized</span>
                </div>
                <div className="flex justify-between border-b border-t1-gray/10 pb-2 pr-10">
                  <span className="font-oswald text-xs tracking-widest text-white uppercase">Country</span>
                  <span className="text-xs uppercase">South Korea</span>
                </div>
              </motion.div>
            )}
            {activeTab === 'shipping' && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                <p className="mb-4">Standard delivery: 2-5 business days. International shipping available (10-15 business days depending on location).</p>
                <p>Returns are accepted within 30 days of purchase. Items must be in original condition with tags attached. Please note that limited edition sale items may be final sale.</p>
              </motion.div>
            )}
          </div>
        </div>
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-32">
            <div className="flex items-center justify-between mb-12">
              <h2 className="text-4xl font-oswald font-black text-white italic uppercase tracking-tighter">You May Also Like</h2>
              <Link to="/shop" className="text-xs font-oswald font-bold text-t1-red tracking-[0.2em] uppercase hover:underline">View All</Link>
            </div>
